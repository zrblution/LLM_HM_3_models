...
class Qwen2_5_CustomVLForConditionalGeneration(nn.Module):
    ...
    def _load_vem_weights_from_checkpoint(self, ckpt_path: str) -> None:
        """Load all weights from checkpoint, handling _model. prefix.
        
        训练时保存的权重键名带有 "_model." 前缀（如 _model.model.language_model.layers.0.xxx），
        但 self._model.load_state_dict 期望的键名不带这个前缀（如 model.language_model.layers.0.xxx）。
        
        这个方法会：
        1. 加载所有带 "_model." 前缀的权重，去掉前缀后加载到 self._model
        2. 特别处理 VEM 模块的权重
        """
        ...
        print(
            f"[Ministral VEM] Loaded weights from checkpoint: "
            f"base_model={base_count}, vem={vem_count}, "
            f"missing={len(missing)}, unexpected={len(unexpected)}"
        )
        
        # 如果还有很多 missing，可能是因为某些权重没有 _model. 前缀
        if len(missing) > 10:
            print(f"[Ministral VEM] Warning: {len(missing)} weights still missing. "
                  f"This may indicate a checkpoint format mismatch.")
