...
class CustomMinistralDecoderLayer(nn.Module):
    ...
    VEM 缓存机制：
    - 使用类变量 _vem_cache 存储 v_mem 和 v_mem_mask
    - 第一层在 forward 时从 hidden_states 中提取视觉 token 作为 v_mem
    - 后续层从缓存中读取 v_mem
    """
    # 类变量：用于在层之间共享 v_mem 缓存
    _vem_cache = None
    _vem_cache_batch_size = None
    # 自动提取 VEM 的控制变量
    _auto_extract_vem = False
    _image_token_index = 10
    _input_ids = None
    _vem_extracted = False  # 标记是否已经提取过 VEM
    
    @classmethod
    def clear_vem_cache(cls):
        """清除 VEM 缓存"""
        cls._vem_cache = None
        cls._vem_cache_batch_size = None
        cls._vem_extracted = False
    
    @classmethod
    def set_vem_cache(cls, v_mem, v_mem_mask, batch_size):
        """设置 VEM 缓存"""
        cls._vem_cache = (v_mem, v_mem_mask)
        cls._vem_cache_batch_size = batch_size
        cls._vem_extracted = True
    
    @classmethod
    def get_vem_cache(cls):
        """获取 VEM 缓存"""
        if cls._vem_cache is None:
            return None, None
        return cls._vem_cache
    
    @classmethod
    def extract_vem_from_hidden_states(cls, hidden_states, input_ids=None):
        """从 hidden_states 中提取视觉 token 作为 v_mem
        
        Args:
            hidden_states: [batch, seq_len, hidden_size]
            input_ids: [batch, seq_len] 用于识别视觉 token 位置
        
        Returns:
            v_mem: [batch, num_visual_tokens, hidden_size]
            v_mem_mask: [batch, num_visual_tokens]
        """
        if input_ids is None:
            input_ids = cls._input_ids
        
        if input_ids is None:
            # 如果没有 input_ids，使用整个 hidden_states 作为 v_mem
            # 这是一个 fallback，可能不是最优的
            return hidden_states, torch.ones(hidden_states.shape[0], hidden_states.shape[1], 
                                             device=hidden_states.device, dtype=torch.long)
        
        bsz, seq_len, hidden_size = hidden_states.shape
        image_token_index = cls._image_token_index
        
        # 找到每个样本中的视觉 token 位置
        # 注意：input_ids 的长度可能与 hidden_states 不同（因为 generate 过程中会变化）
        if input_ids.shape[1] != seq_len:
            # 如果长度不匹配，只使用 input_ids 的前 seq_len 个 token
            # 或者使用 hidden_states 的前 input_ids.shape[1] 个 token
            min_len = min(input_ids.shape[1], seq_len)
            input_ids_truncated = input_ids[:, :min_len]
            hidden_states_truncated = hidden_states[:, :min_len, :]
        else:
            input_ids_truncated = input_ids
            hidden_states_truncated = hidden_states
        
        # 找到视觉 token 的位置
        visual_mask = (input_ids_truncated == image_token_index)  # [batch, seq_len]
        
        # 计算每个样本的视觉 token 数量
        num_visual_per_sample = visual_mask.sum(dim=1)  # [batch]
        max_visual = num_visual_per_sample.max().item()
        
        if max_visual == 0:
            # 没有视觉 token，返回 None
            return None, None
        
        # 提取视觉 token 的 hidden_states
        v_mem = hidden_states_truncated.new_zeros((bsz, max_visual, hidden_size))
        v_mem_mask = hidden_states_truncated.new_zeros((bsz, max_visual), dtype=torch.long)
        
        for i in range(bsz):
            visual_indices = visual_mask[i].nonzero(as_tuple=True)[0]
            n_visual = len(visual_indices)
            if n_visual > 0:
                v_mem[i, :n_visual, :] = hidden_states_truncated[i, visual_indices, :]
                v_mem_mask[i, :n_visual] = 1
        
        return v_mem, v_mem_mask
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 如果没有传入 v_mem，尝试从类缓存中获取或自动提取
        if v_mem is None:
            cached_v_mem, cached_v_mem_mask = CustomMinistralDecoderLayer.get_vem_cache()
            if cached_v_mem is not None:
                v_mem = cached_v_mem
                v_mem_mask = cached_v_mem_mask
            elif (CustomMinistralDecoderLayer._auto_extract_vem and 
                  not CustomMinistralDecoderLayer._vem_extracted and 
                  self.layer_idx == 0):
                # 第一层且尚未提取 VEM，从 hidden_states 中提取
                v_mem, v_mem_mask = CustomMinistralDecoderLayer.extract_vem_from_hidden_states(
                    hidden_states, CustomMinistralDecoderLayer._input_ids
                )
                if v_mem is not None:
                    # 缓存提取的 VEM 供后续层使用
                    CustomMinistralDecoderLayer.set_vem_cache(v_mem, v_mem_mask, hidden_states.shape[0])
        ...
        # 调试输出（只在第一层输出一次）
        if self.layer_idx == 0 and not hasattr(CustomMinistralDecoderLayer, '_debug_printed'):
            print(f"[VEM DEBUG] Layer 0: enable_vision_gate={enable_vision_gate}, v_mem is None={v_mem is None}, "
                  f"auto_extract={CustomMinistralDecoderLayer._auto_extract_vem}, "
                  f"vem_extracted={CustomMinistralDecoderLayer._vem_extracted}")
            if v_mem is not None:
                print(f"[VEM DEBUG] v_mem shape: {v_mem.shape}")
            CustomMinistralDecoderLayer._debug_printed = True
...
class Qwen2_5_CustomVLForConditionalGeneration(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        # 加载 VEM 模块的权重（如果存在）
        vem_loaded_count = instance._load_custom_layers_weights(pretrained_model_name_or_path)
        
        # 打印 LOAD REPORT（类似 Qwen 模型的输出）
        instance._print_load_report(pretrained_model_name_or_path, vem_loaded_count)
        
        return instance
    
    def _print_load_report(self, model_path, vem_loaded_count=0):
        ...
        # 根据是否成功加载权重决定状态
        status = "✅ LOADED" if vem_loaded_count > 0 else "MISSING (newly initialized)"
        ...
        for param in vem_params:
            ...
            if len(parts) >= 4:
                ...
                param_groups[key] = status
        
        # 打印报告
        print(f"\nMinistralCustomVLForConditionalGeneration LOAD REPORT from: {model_path}")
        print("-" * 80)
        print(f"{'Key':<65} | {'Status':<20} |")
        print("-" * 80)
        
        for key, s in sorted(param_groups.items()):
            print(f"{key:<65} | {s:<20} |")
        
        print("-" * 80)
        if vem_loaded_count > 0:
            print(f"\nNotes:")
            print(f"- ✅ LOADED: VEM modules were successfully loaded from checkpoint.")
            print(f"- Total VEM parameters: {len(vem_params)} across {num_layers} layers.")
            print(f"- Loaded {vem_loaded_count} VEM parameters.\n")
        else:
            print(f"\nNotes:")
    ...
    def _load_custom_layers_weights(self, model_path):
        ...
        for sf_path in safetensors_files:
            try:
                ...
                # 筛选出 custom_layers 相关的权重，并移除 'custom_layers.' 前缀
                # 因为 self.custom_layers 是 nn.ModuleList，期望的键名格式是 '0.xxx' 而不是 'custom_layers.0.xxx'
                custom_weights = {}
                for k, v in state_dict.items():
                    if k.startswith('custom_layers.'):
                        # 移除 'custom_layers.' 前缀
                        new_key = k[len('custom_layers.'):]
                        custom_weights[new_key] = v
    ...
    def _wrap_decoder_layers(self):
        ...
        # 关键：替换原始模型中的 decoder layers，使得 generate() 也能使用 VEM
        # 这样 self._model.generate() 内部调用 forward 时会使用带 VEM 的层
        if hasattr(self._model, 'model') and hasattr(self._model.model, 'language_model'):
            lm = self._model.model.language_model
            if hasattr(lm, 'layers'):
                lm.layers = self.custom_layers
                print(f"✅ Replaced decoder layers in model.model.language_model.layers with VEM-wrapped layers")
        elif hasattr(self._model, 'language_model'):
            lm = self._model.language_model
            if hasattr(lm, 'layers'):
                lm.layers = self.custom_layers
                print(f"✅ Replaced decoder layers in language_model.layers with VEM-wrapped layers")
            elif hasattr(lm, 'model') and hasattr(lm.model, 'layers'):
                lm.model.layers = self.custom_layers
                print(f"✅ Replaced decoder layers in language_model.model.layers with VEM-wrapped layers")
        elif hasattr(self._model, 'model') and hasattr(self._model.model, 'layers'):
            self._model.model.layers = self.custom_layers
            print(f"✅ Replaced decoder layers in model.layers with VEM-wrapped layers")
    ...
    def generate(self, *args, **kwargs):
        ...
        VEM 模块会在 decoder layer 的 forward 中自动从 hidden_states 提取视觉信息。
        这里我们只需要确保在第一次 forward 时设置好 v_mem 缓存。
        """
        # 清除之前的缓存
        CustomMinistralDecoderLayer.clear_vem_cache()
        
        # 启用 VEM 自动提取模式
        # 第一层会在 forward 时从 hidden_states 中提取视觉 token 作为 v_mem
        if self.custom_layers is not None:
            CustomMinistralDecoderLayer._auto_extract_vem = True
            # 获取 image_token_index 用于识别视觉 token
            image_token_index = getattr(self.config, 'image_token_index', 10)
            CustomMinistralDecoderLayer._image_token_index = image_token_index
            # 传递 input_ids 用于识别视觉 token 位置
            input_ids = kwargs.get('input_ids', None)
            if input_ids is not None:
                CustomMinistralDecoderLayer._input_ids = input_ids
        ...
        try:
            ...
            # 生成完成后清除缓存和状态
            CustomMinistralDecoderLayer.clear_vem_cache()
            CustomMinistralDecoderLayer._auto_extract_vem = False
            CustomMinistralDecoderLayer._input_ids = None
