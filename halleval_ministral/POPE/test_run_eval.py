#!/usr/bin/env python3
"""
Evaluation runner for POPE using test datasets (test_gqa and test_coco).

Usage examples:
  python test_run_eval.py --dataset test_coco --model_dir /home/tos_data/LLM-Disentanglement-Hallucination-Mitigation/output_model/checkpoint-6813
  python test_run_eval.py --dataset test_gqa --model_dir /home/tos_data/LLM-Disentanglement-Hallucination-Mitigation/output_model/checkpoint-6813
  CUDA_VISIBLE_DEVICES=3 python test_run_eval.py --dataset test_coco --model_dir /home/tos_data/LLM-Disentanglement-Hallucination-Mitigation/output_model/checkpoint-6813 --gpu 3
...
For test_gqa and test_coco datasets, it uses the corresponding test directories.
"""
import argparse
import os
import subprocess
...
def eval_dataset(dataset: str, model_dir: str, model_name: str = None, gpu: str = None, dry_run: bool = False, use_vcd: bool = False, use_inter: bool = False):
    ...
    # Determine base dataset name (gqa or coco) from test dataset name
    if dataset == "test_gqa":
        base_dataset = "gqa"
    elif dataset == "test_coco":
        base_dataset = "coco"
    else:
        raise ValueError(f"Unsupported test dataset: {dataset}. Use 'test_gqa' or 'test_coco'.")

    # Centralized result layout: POPE/result/{model_name}/{dataset}/...
    result_dir = ROOT / "result" / model_name / dataset
    ensure_dir(result_dir)

    # Use test dataset directories
    ...
    for variant in VARIANTS:
        json_filename = f"{base_dataset}_pope_{variant}.json"
    ...
    if not variants_pairs:
        raise RuntimeError(f"No variants detected for dataset '{dataset}' at {output_root}. Please check if the json files exist.")
