"""
Ministral-3-3B-Instruct-2512 原始模型基线训练脚本
不添加任何新模块，直接使用原始 Mistral3ForConditionalGeneration 进行微调
"""
import torch
...
from transformers import (
    ...
    Trainer,
    AutoProcessor,
    Mistral3ForConditionalGeneration  # 直接使用原始模型
)
...
class ModelArguments:
    ...
    # 以下参数与 train_ministral.py 保持一致，基线模型不使用但保留以兼容命令行
...
# ========== 关键修改：直接使用原始 Mistral3ForConditionalGeneration ==========
print("Loading original Mistral3ForConditionalGeneration model (no custom modules)...")
...
class EvidenceTrainer(Trainer):
    """
    自定义 Trainer，与 train_ministral.py 保持一致
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 但保留这部分代码以确保训练逻辑与 train_ministral.py 完全一致
